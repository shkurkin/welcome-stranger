...
class Wstopmenu extends Module
{
	private $_menu = '';
    private $_rightmenu = '';
	private $_html = '';
	...
	public function install()
	{
		...
		if (!parent::install() ||
			...
			!Configuration::updateGlobalValue('MOD_WSTOPMENU_ITEMS', 'CAT1,CMS1,CMS2,PRD1') ||
			!Configuration::updateGlobalValue('MOD_WSTOPMENU_SEARCH', '1') ||
	...
	{
		if (!parent::uninstall() ||
			!Configuration::deleteByName('MOD_WSTOPMENU_ITEMS') ||
			!Configuration::deleteByName('MOD_WSTOPMENU_SEARCH') ||
			!$this->uninstallDB())
			return false;
		return true;
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitWstopmenu'))
		{
			if (Configuration::updateValue('MOD_WSTOPMENU_ITEMS', Tools::getValue('items')))
			...
			Configuration::updateValue('MOD_WSTOPMENU_SEARCH', (bool)Tools::getValue('search'));
			$update_cache = true;
		}
		else if (Tools::isSubmit('submitWstopmenuLinks'))
		{
			...
			{
				WsMenuTopLinks::add(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
				$this->_html .= $this->displayConfirmation($this->l('The link has been added.'));
			}
			$update_cache = true;
		}
		else if (Tools::isSubmit('submitWstopmenuRemove'))
		{
			$id_linksmenutop = Tools::getValue('id_linksmenutop', 0);
			WsMenuTopLinks::remove($id_linksmenutop, (int)Shop::getContextShopID());
			Configuration::updateValue('MOD_WSTOPMENU_ITEMS', str_replace(array('LNK'.$id_linksmenutop.',', 'LNK'.$id_linksmenutop), '', Configuration::get('MOD_WSTOPMENU_ITEMS')));
			$this->_html .= $this->displayConfirmation($this->l('The link has been removed'));
			$update_cache = true;
		}
		else if (Tools::isSubmit('submitWstopmenuEdit'))
		{
			...
			{
				$tmp = WsMenuTopLinks::getLinkLang($id_linksmenutop, $id_shop);
			...
			{
				WsMenuTopLinks::update(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)$id_shop, (int)$id_linksmenutop, (int)$id_linksmenutop);
		...
		<fieldset>
			...
			<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post" id="form">
				...
				<div class="margin-form">
					<input type="text" name="items" id="itemsInput" value="'.Tools::safeOutput(Configuration::get('MOD_WSTOPMENU_ITEMS')).'" size="70" />
				</div>
				</div>
		...
		$this->_html .= '</optgroup>';
        print "about to make top links";
		// BEGIN Menu Top Links
		$this->_html .= '<optgroup label="'.$this->l('Menu Top Links').'">';
		$links = WsMenuTopLinks::gets($id_lang, null, (int)Shop::getContextShopID());
		foreach ($links as $link)
		{
			if ($link['label'] == '')
			{
				$link = WsMenuTopLinks::get($link['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
		...
		$this->_html .= '</select><br/>
				...
				<div class="margin-form">
					<input type="checkbox" name="search" id="s" value="1"'.((Configuration::get('MOD_WSTOPMENU_SEARCH')) ? ' checked=""': '').'/>
				</div>
				<p class="center">
					<input type="submit" name="submitWstopmenu" value="'.$this->l('Save	').'" class="button" />
				</p>
			</form>
		</fieldset><br />';
...
<div class="margin-form">';
		...
		if (Tools::isSubmit('submitWstopmenuEdit'))
			$this->_html .= '<input type="submit" name="submitWstopmenuEdit" value="'.$this->l('Edit').'" class="button" />';

		$this->_html .= '
					<input type="submit" name="submitWstopmenuLinks" value="'.$this->l('Add	').'" class="button" />
</div>

			</form>
		</fieldset><br />';

		$links = WsMenuTopLinks::gets((int)$id_lang, null, (int)Shop::getContextShopID());
					...
					<tr>
						...
						<td>
							<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
								...
								<input type="submit" name="submitWstopmenuEdit" value="'.$this->l('Edit').'" class="button" />
								<input type="submit" name="submitWstopmenuRemove" value="'.$this->l('Remove').'" class="button" />
							</form>
						</td>
					</tr>';
		}
	...
	{
		return explode(',', Configuration::get('MOD_WSTOPMENU_ITEMS'));
	}
	...
	private function makeMenuOption()
	{
		...
		foreach ($menu_item as $item)
		{
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					$link = WsMenuTopLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						if (!isset($link[0]['label']) || ($link[0]['label'] == ''))
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = WsMenuTopLinks::get($link[0]['id_linksmenutop'], (int)$default_language, (int)Shop::getContextShopID());
						}
    ...
    private function makeRightMenu(){
	...
	private function makeMenu()
	{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = WsMenuTopLinks::get($link[0]['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
						}
        ...
        if(substr($this->_menu, -8) == ' / </li>'){
            $this->_menu = substr($this->_menu, 0, -8) . '</li>';
        }
	...
	private function getCategory($id_category, $id_lang = false, $id_shop = false, $hideCarat = false)
	{
		$id_lang = $id_lang ? (int)$id_lang : (int)Context::getContext()->language->id;
        ...
        if($category->name == 'Home'){
            ...
            if (!empty($is_intersected) && (!isset($this->usedCategoryIds[$category->id]))){
		$this->usedCategoryIds[$category->id] = 1;
                    if(empty($children)){
			$this->_menu .= '<li '.$selected.'>';
			$this->_menu .= '<a href="#a'.$category->name.'" class="aonpagetarget scroll">'.$category->name.'</a>';
			$this->_menu .= '</li>';
                        if(!$hideCarat){
                            $this->_menu .= '<li class="noHover"> / </li>';
                        }
                    } else {
                        $this->_menu .= '<li class="dropdown">';
                        $this->_menu .= '<a class="dropdown-toggle" data-toggle="dropdown" href="#">'.$category->name.'<span class="caret"></span></a>';
                        $this->_menu .= '<ul class="dropdown-menu">';
                        
                        foreach($children as $child){
                            $this->getCategory((int)$child['id_category'], (int)$id_lang, (int)$child['id_shop'], true);
                        }
                        $this->_menu .= '</ul>';
                        $this->_menu .= '</li>';
                        if(!$hideCarat){
	...
	private function getCMSMenuItems($parent, $depth = 1, $id_lang = false)
	{
		...
		if (count($categories) || count($pages))
		{
			...
			foreach ($pages as $page)
			{
				...
				$this->_menu .= '<li '.$selected.'>';
				$this->_menu .= '<a href="'.$links[0]['link'].'">'.$cms->meta_title.'1</a>';
				$this->_menu .= '</li>';
			}
	...
	protected function getCacheId($name = null)
	{
		...
		return 'wstopmenu|'.(int)Tools::usingSecureMode().'|'.$page_name.'|'.(int)$this->context->shop->id.'|'.implode(', ',$this->user_groups).'|'.(int)$this->context->language->id.'|'.(int)Tools::getValue('id_category').'|'.(int)Tools::getValue('id_manufacturer').'|'.(int)Tools::getValue('id_supplier').'|'.(int)Tools::getValue('id_cms').'|'.(int)Tools::getValue('id_product');
	}
	...
	public function hookDisplayTop($param)
	{
		...
		if (!$this->isCached('wstopmenu.tpl', $this->getCacheId()))
		{
			$this->makeMenu();
                        $this->makeRightMenu();
			$this->smarty->assign('MENU_SEARCH', Configuration::get('MOD_WSTOPMENU_SEARCH'));
			$this->smarty->assign('MENU', $this->_menu);
                        $this->smarty->assign('RIGHTMENU', $this->_rightmenu);
		...
		$html = $this->display(__FILE__, 'wstopmenu.tpl', $this->getCacheId());
		return $html;
	}
	...
	{
		$this->_clearCache('wstopmenu.tpl');
	}
