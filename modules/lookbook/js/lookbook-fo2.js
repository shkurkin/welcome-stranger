var attributeManipulation = function(jsonAttributes)
{
	...
	amThis.productPrice;
    amThis.productBasePriceTaxExcluded;
	...
	this.addCombination = function(arrayKey)
	{
		...
		if(typeof(jsonAttributes[arrayKey]) == 'object')
		{
			for(var attribute in jsonAttributes[arrayKey].combinations)
			{
				...
				combination['specific_price'] = jsonAttributes[arrayKey].combinations[attribute].specific_price;
	...
	this.findCombination = function()
	{
			...
			for (var combination = 0; combination < amThis.combinations.length; ++combination)
			{
				...
				if (combinationMatchForm)
				{
                    ...
                    amThis.selectedCombination['specific_price'] = amThis.combinations[combination]['specific_price'];
...
//						displayImage( $('#thumb_'+amThis.combinations[combination]['image']).parent() );
	...
	{
        var productPriceDisplay = amThis.productPrice;
		var productPriceWithoutReductionDisplay = amThis.productPriceWithoutReduction;
		...
		if (!amThis.selectedCombination['unavailable'] && amThis.productShowPrice == 1)
		{
			...
			priceTaxExclWithoutGroupReduction = ps_round(amThis.productPriceTaxExcluded, 6) * (1 / amThis.group_reduction);

			var tax = (amThis.taxRate / 100) + 1;

			var taxExclPrice = priceTaxExclWithoutGroupReduction + (amThis.selectedCombination['price'] * amThis.currencyRate);

			if (amThis.selectedCombination.specific_price && amThis.selectedCombination.specific_price['id_product_attribute'])
			{
				if (amThis.selectedCombination.specific_price['price'] && amThis.selectedCombination.specific_price['price'] >= 0) {
					var taxExclPrice = (amThis.specific_currency ? amThis.selectedCombination.specific_price['price'] : amThis.selectedCombination.specific_price['price'] * amThis.currencyRate);
                }
				else {
					var taxExclPrice = amThis.productBasePriceTaxExcluded * amThis.currencyRate + (amThis.selectedCombination['price'] * amThis.currencyRate);
                }
			}
			else if (amThis.product_specific_price.price && amThis.product_specific_price.price >= 0) {
				var taxExclPrice = (amThis.specific_currency ? amThis.product_specific_price.price : amThis.product_specific_price.price * amThis.currencyRate) + (amThis.selectedCombination['price'] * amThis.currencyRate);
			}
			if (!amThis.displayPrice && !amThis.noTaxForThisProduct) {
                amThis.productPriceDisplay = taxExclPrice * tax; // Need to be global => no var
            }
			else {
				amThis.productPriceDisplay = ps_round(taxExclPrice, 2); // Need to be global => no var
			}

			amThis.productPriceWithoutReductionDisplay = amThis.productPriceDisplay * amThis.group_reduction;
			var reduction = 0;

			if (amThis.selectedCombination['specific_price'].reduction_price || amThis.selectedCombination['specific_price'].reduction_percent)
			{
				reduction_price = (amThis.specific_currency ? amThis.selectedCombination['specific_price'].reduction_price : amThis.selectedCombination['specific_price'].reduction_price * amThis.currencyRate);
				reduction = amThis.productPriceDisplay * (parseFloat(amThis.selectedCombination['specific_price'].reduction_percent) / 100) + amThis.reduction_price;
				if (amThis.reduction_price && (amThis.displayPrice || amThis.noTaxForThisProduct))
					amThis.reduction = ps_round(amThis.reduction / tax, 6);

			}
			else if (amThis.product_specific_price && amThis.product_specific_price.reduction)
			{
				if (amThis.product_specific_price.reduction_type == 'amount')
					amThis.reduction_price = (amThis.specific_currency ? amThis.product_specific_price.reduction : amThis.product_specific_price.reduction * amThis.currencyRate);
				else
					amThis.reduction_price = 0;

				if (amThis.product_specific_price.reduction_type == 'percentage')
					amThis.reduction_percent = amThis.productPriceDisplay * parseFloat(amThis.product_specific_price.reduction);

				amThis.reduction = amThis.reduction_price + amThis.reduction_percent;
				if (amThis.reduction_price && (amThis.displayPrice || amThis.noTaxForThisProduct))
					amThis.reduction = ps_round(amThis.reduction / tax, 6);
			}

			if (amThis.selectedCombination.specific_price)
			{
				if (amThis.selectedCombination['specific_price'] && amThis.selectedCombination['specific_price'].reduction_type == 'percentage')
				{
					$('#reduction_amount'+'_'+amThis.arrayKey).hide();
					$('#reduction_percent_display'+'_'+amThis.arrayKey).html('-' + parseFloat(amThis.selectedCombination['specific_price'].reduction_percent) + '%');
					$('#reduction_percent'+'_'+amThis.arrayKey).show();
				} else if (amThis.selectedCombination['specific_price'].reduction_type == 'amount' && amThis.selectedCombination['specific_price'].reduction_price != 0) {
					$('#reduction_amount_display'+'_'+amThis.arrayKey).html('-' + formatCurrency(amThis.reduction_price, amThis.currencyFormat, amThis.currencySign, amThis.currencyBlank));
					$('#reduction_percent'+'_'+amThis.arrayKey).hide();
					$('#reduction_amount'+'_'+amThis.arrayKey).show();
				} else {
					$('#reduction_percent'+'_'+amThis.arrayKey).hide();
					$('#reduction_amount'+'_'+amThis.arrayKey).hide();
				}
			...
			if (amThis.product_specific_price['reduction_type'] != '' || amThis.selectedCombination['specific_price'].reduction_type != '')
				$('#discount_reduced_price'+'_'+amThis.arrayKey+',#old_price'+'_'+amThis.arrayKey).show();
			else
				$('#discount_reduced_price'+'_'+amThis.arrayKey+',#old_price'+'_'+amThis.arrayKey).hide();
			if ((amThis.product_specific_price['reduction_type'] == 'percentage' && amThis.selectedCombination['specific_price'].reduction_type == 'percentage') || amThis.selectedCombination['specific_price'].reduction_type == 'percentage')
				$('#reduction_percent'+'_'+amThis.arrayKey).show();
			else
				$('#reduction_percent'+'_'+amThis.arrayKey).hide();
			if (amThis.product_specific_price['price'] || (amThis.selectedCombination.specific_price && amThis.selectedCombination.specific_price['price']))
				$('#not_impacted_by_discount'+'_'+amThis.arrayKey).show();
			else
				$('#not_impacted_by_discount'+'_'+amThis.arrayKey).hide();

            if(amThis.reduction != undefined) {
			    amThis.productPriceDisplay -= amThis.reduction;
            }

			var tmp = productPriceDisplay * amThis.group_reduction;
			amThis.productPriceDisplay = ps_round(amThis.productPriceDisplay * amThis.group_reduction, 2);
            ...
            if(amThis.ecotaxAmount != undefined) {
                amThis.productPriceDisplay += amThis.ecotaxAmount;
                amThis.productPriceWithoutReductionDisplay += amThis.ecotaxAmount;
            }

			var our_price = '';
			if (amThis.productPriceDisplay > 0) {
				our_price = formatCurrency(amThis.productPriceDisplay, amThis.currencyFormat, amThis.currencySign, amThis.currencyBlank);
			} else {
				our_price = formatCurrency(0, amThis.currencyFormat, amThis.currencySign, amThis.currencyBlank);
			}
			$('#our_price_display'+'_'+amThis.arrayKey).text(our_price);
			$('#old_price_display'+'_'+amThis.arrayKey).text(formatCurrency(amThis.productPriceWithoutReductionDisplay, amThis.currencyFormat, amThis.currencySign, amThis.currencyBlank));

			if (amThis.productPriceWithoutReductionDisplay > amThis.productPriceDisplay)
				$('#old_price'+'_'+amThis.arrayKey+',#old_price_display'+'_'+amThis.arrayKey+',#old_price_display_taxes'+'_'+amThis.arrayKey).show();
			else
				$('#old_price'+'_'+amThis.arrayKey+',#old_price_display'+'_'+amThis.arrayKey+',#old_price_display_taxes'+'_'+amThis.arrayKey).hide();
			// Special feature: "Display product price tax excluded on product page"
			var productPricePretaxed = '';
			if (!amThis.noTaxForThisProduct)
				amThis.productPricePretaxed = amThis.productPriceDisplay / tax;
			else
				amThis.productPricePretaxed = amThis.productPriceDisplay;
			$('#pretaxe_price_display'+'_'+amThis.arrayKey).text(formatCurrency(amThis.productPricePretaxed, amThis.currencyFormat, amThis.currencySign, amThis.currencyBlank));
			// Unit price 
			amThis.productUnitPriceRatio = parseFloat(amThis.productUnitPriceRatio);
			if (amThis.productUnitPriceRatio > 0 )
			{
				amThis.newUnitPrice = (amThis.productPriceDisplay / parseFloat(amThis.productUnitPriceRatio)) + amThis.selectedCombination['unit_price'];
				$('#unit_price_display'+'_'+amThis.arrayKey).text(formatCurrency(amThis.newUnitPrice, amThis.currencyFormat, amThis.currencySign, amThis.currencyBlank));
			}

			// Ecotax
			amThis.ecotaxAmount = !amThis.displayPrice ? ps_round(amThis.selectedCombination['ecotax'] * (1 + amThis.ecotaxTax_rate / 100), 2) : amThis.selectedCombination['ecotax'];
			$('#ecotax_price_display'+'_'+amThis.arrayKey).text(formatCurrency(amThis.ecotaxAmount, amThis.currencyFormat, amThis.currencySign, amThis.currencyBlank));
		}
