...
include_once(_PS_MODULE_DIR_ . 'wshomeslider/WsHomeSlide.php');

class WsHomeSlider extends Module {

    private $_html = '';

    public function __construct() {
        $this->name = 'wshomeslider';
        $this->tab = 'front_office_features';
        $this->version = '1.2.2';
        $this->author = 'Nathan Hillen';
        $this->need_instance = 0;
        $this->secure_key = Tools::encrypt($this->name);

        parent::__construct();

        $this->displayName = $this->l('WS Modified Image slider for your homepage.');
        $this->description = $this->l('Adds a modified image slider to your homepage.');
    }
    ...
    protected function deleteTables() {
        ...
        if ($slides) {
            ...
            foreach ($slides as $slide) {
                $to_del = new WsHomeSlide($slide['id_slide']);
                $to_del->delete();
            }
    ...
    private function _displayAddForm() {
        ...
        if (Tools::isSubmit('id_slide') && $this->slideExists((int) Tools::getValue('id_slide')))
            $slide = new WsHomeSlide((int) Tools::getValue('id_slide'));
    ...
    private function _postProcess() {
        ...
        elseif (Tools::isSubmit('delete_id_slide')) {
            $slide = new WsHomeSlide((int) Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res)
    ...
    private function _prepareHook() {
        if (!$this->isCached('wshomeslider.tpl', $this->getCacheId())) {
            $slider = array(
    ...
    public function hookDisplayHome() {
        ...
        return $this->display(__FILE__, 'wshomeslider.tpl', $this->getCacheId());
    }

    public function clearCache() {
        $this->_clearCache('wshomeslider.tpl');
    }
