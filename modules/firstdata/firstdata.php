...
class Firstdata extends PaymentModule
{
	...
	public function hookAdminOrder($params)
	{
		...
		'firstdata_date_cancel' => $transaction['date_cancel'],
                    'tempDump' => print_r($transaction_details,1 ),
	...
	public function validation()
	{
		...
		{
			$result = $this->_firstDataCall('{"gateway_id": "'.Configuration::get('FIRSTDATA_GATEWAY_ID').'", "password": "'.Configuration::get('FIRSTDATA_PASSWORD').'", "transaction_type": "01", "amount": "'.(float)$cart->getOrderTotal().'", "cc_number": "'.Tools::safeOutput(Tools::getValue('x_card_num')).'", "cc_expiry": "'.(Tools::getValue('x_exp_date_m') < 10 ? '0'.(int)Tools::getValue('x_exp_date_m') : (int)Tools::getValue('x_exp_date_m')).(int)Tools::getValue('x_exp_date_y').'", "cardholder_name": "'.Tools::safeOutput(Tools::getValue('firstdata_card_holder')).'"}');
			$json_result = Tools::jsondecode($result);
			if (isset($json_result->transaction_approved) && $json_result->transaction_approved)
			{
				$this->_insertTransaction(array('id_cart' => (int)$cart->id, 'authorization_num' => pSQL($json_result->authorization_num), 'transaction_tag' => (int)$json_result->transaction_tag, 'date_add' => date('Y-m-d H:i:s'), 'status'=>'authorized'));
	...
	private function _firstDataCall($data_string)
	{
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://api.demo.globalgatewaye4.firstdata.com/transaction/v12"); //'https://api.globalgatewaye4.firstdata.com/transaction/v12'); // Sandbox: https://api.demo.globalgatewaye4.firstdata.com/transaction/v12
        ...
        public function sendCapture($orderId){
            $order = new Order($orderId);

            $transaction = Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'firstdata` WHERE `id_cart` = '.(int)$order->id_cart . ' AND status="authorized"');
            
            $result = $this->_firstDataCall(
                    '{"gateway_id": "'.Configuration::get('FIRSTDATA_GATEWAY_ID').'", '
            ...
            if (isset($json_result->transaction_approved) && $json_result->transaction_approved){
                Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'firstdata` SET status="captured" WHERE `id_cart` = '.(int)$order->id_cart);
                return true;
            } else {
                return false;
            }
