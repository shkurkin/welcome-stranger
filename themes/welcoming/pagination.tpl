...
{if isset($p) AND $p}
	...
	{if $start!=$stop}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_next"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}">{l s='Next'}&nbsp;</a></li>
		{else}
			<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="disabled pagination_next"><span>{l s='Next'}&nbsp;</span></li>
		{/if}
		</ul>
	{/if}
