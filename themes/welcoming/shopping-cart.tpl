...
{else}
    ...
    {/if}
    <p>{l s='Your shopping cart contains '} <span id="summary_products_quantity"><b>{$productNumber}</b> {if $productNumber == 1}{l s='item'}{else}{l s='items'}{/if}</span></p>
    <div id="order-detail-content" class="table_block">
        ...
        <tr class="cart_total_price">
                        ...
                        <div id="display_cart_vouchers">
                            {foreach $displayVouchers as $voucher}
                                {if $voucher.code != ''}<span onclick="$('#discount_name').val('{$voucher.code}');
                                        return false;" class="voucher_name">{$voucher.code}</span> - {/if}{$voucher.name}<br />
                            {/foreach}
                        </div>
                    {/if}
                {/if}
            ...
            {if $use_taxes}
                <td class="total_price_container" id="total_price_container"><p>{l s='Total'}</p></td>
                <td class="price final_price">{displayPrice price=$total_price}</td>
            {else}
                <td class="total_price_container" id="total_price_container"><p>{l s='Total'}</p></td>
                <td class="price final_price">{displayPrice price=$total_price_without_tax}</td>
            {/if}
        </tr>
    </tfoot>
    <tbody>
